...
def start_timer():
    ...
    if reps % 8 == 0:
        ...
        title_label.config(text="Break", fg=PINK , bg=YELLOW, font=(FONT_NAME, 50, "bold"))
    else:
        count_down(work_sec)
...
def count_down(count):
    ...
    if count > 0:
        ...
        start_timer()
        if reps % 2 == 0:
            tick_label.config(text=f"{'✔'*int(reps / 2)}", bg=YELLOW, fg=GREEN, font=(20))
...
strt_btn = Button(text="Start", bd=5, highlightthickness=0)
strt_btn.config(command=start_timer)
strt_btn.grid(column=0,row=2)

tick_label = Label(bg=YELLOW)
tick_label.grid(column=1,row=3)

reset_btn = Button(text="Reset", bd=5, highlightthickness=0, command=reset_timer)
